...
export function SettingsModal({
  ...
  return (
    ...
    >
      <div className="vos-panel vos-panel-shadow flex h-[min(560px,100%)] w-[min(720px,100%)] overflow-hidden rounded-2xl">
        <nav className="flex w-56 shrink-0 flex-col gap-0.5 border-r border-[var(--vos-border)] p-3">
          <span className="px-2 pb-2 pt-1 text-[15px] font-semibold text-[var(--vos-text-strong)]">
            Einstellungen
          </span>
          {TABS.map((t) => (
            <button
        ...
        <div className="relative flex-1 overflow-y-auto p-6">
          <button
            ...
            className="absolute right-4 top-4 grid h-8 w-8 place-items-center rounded-lg text-[var(--vos-muted)] hover:bg-[var(--vos-hover-soft)] hover:text-[var(--vos-text-strong)]"
          >
            <CloseIcon size={18} />
          </button>
